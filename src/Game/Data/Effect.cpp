...
void Effect::initFromIni(INIReader * ini, const std::string & section)
{
	...
	user = nullptr;
	target = nullptr;
	fileName = ini->Get(section, u8"FileName", u8"");
	doing = ini->GetInteger(section, u8"Doing", ekExploding);
	position.x = ini->GetInteger(section, u8"MapX", 0);
	position.y = ini->GetInteger(section, u8"MapY", 0);
	flyingDirection.x = ini->GetInteger(section, u8"DirectionX", 0);
	flyingDirection.y = ini->GetInteger(section, u8"DirectionY", 0);
	offset.x = ini->GetReal(section, u8"OffsetX", 0.0);
	offset.y = ini->GetReal(section, u8"OffsetY", 0.0);
	src.x = ini->GetInteger(section, u8"SrcX", 0);
	src.y = ini->GetInteger(section, u8"SrcY", 0);
	srcOffset.x = ini->GetReal(section, u8"SrcOffsetX", 0);
	srcOffset.y = ini->GetReal(section, u8"SrcOffsetY", 0);
	dest.x = ini->GetInteger(section, u8"DestX", 0);
	dest.y = ini->GetInteger(section, u8"DestY", 0);
	destOffset.x = ini->GetReal(section, u8"DestOffsetX", 0);
	destOffset.y = ini->GetReal(section, u8"DestOffsetY", 0);
	beginTime = ini->GetInteger(section, u8"BeginTime", 0);
	beginTime = getTime() - beginTime;
	lifeTime = ini->GetInteger(section, u8"LifeTime", 0);
	waitTime = ini->GetInteger(section, u8"WaitTime", 0);
	damage = ini->GetInteger(section, u8"Damage", 0);
	evade = ini->GetInteger(section, u8"Evade", 0);
	launcherKind = ini->GetInteger(section, u8"Launcher", lkSelf);
	direction = ini->GetInteger(section, u8"Direction", 0);
...
void Effect::saveToIni(INIReader * ini, const std::string & section)
{
	...
	ini->Set(section, u8"FileName", fileName);
	ini->SetInteger(section, u8"Doing", doing);
	ini->SetInteger(section, u8"MapX", position.x);
	ini->SetInteger(section, u8"MapY", position.y);
	ini->SetInteger(section, u8"DirectionX", flyingDirection.x);
	ini->SetInteger(section, u8"DirectionY", flyingDirection.y);
	ini->SetInteger(section, u8"SrcX", src.x);
	ini->SetInteger(section, u8"SrcY", src.y);
	ini->SetReal(section, u8"SrcOffsetX", srcOffset.x);
	ini->SetReal(section, u8"SrcOffsetY", srcOffset.y);
	ini->SetInteger(section, u8"DestX", dest.x);
	ini->SetInteger(section, u8"DestY", dest.y);
	ini->SetReal(section, u8"DestOffsetX", destOffset.x);
	ini->SetReal(section, u8"DestOffsetY", destOffset.y);
	ini->SetReal(section, u8"OffsetX", offset.x);
	ini->SetReal(section, u8"OffsetY", offset.y);
	
	auto tempBeginTime = getTime() - beginTime;
	ini->SetTime(section, u8"BeginTime", tempBeginTime);
	ini->SetTime(section, u8"LifeTime", lifeTime);
	ini->SetTime(section, u8"WaitTime", waitTime);
	ini->SetInteger(section, u8"Damage", damage);
	ini->SetInteger(section, u8"Evade", evade);
	ini->SetInteger(section, u8"Launcher", launcherKind);
	ini->SetInteger(section, u8"Direction", direction);
