...
void Weather::drawElementLum()
{
	...
	for (int i = cenTile.y - yscal; i < cenTile.y + yscal + tileHeightScal; i++)
	{
		...
		{		
			if (gm->map.data == NULL || j < 0 || j >= gm->map.data->head.width || i < 0 || i >= gm->map.data->head.height)
			{
				continue;
			}
...
void Weather::drawWeather()
{
	for (int i = 0; i < (int)drops.size(); i++)
	{
		...
		{
			engine->drawImage(snowflake, (int)drops[i].x, (int)drops[i].y);
		}
		else if (drops[i].type != wtNone)
		{
			unsigned char a = drops[i].dropAlpha;
			int h = 0;
			engine->getWindowSize(NULL, &h);
			...
			engine->drawImage(raindrop, (int)drops[i].x, (int)drops[i].y);
		}
	}
...
{
	unsigned int t = getFrameTime();
	for (int i = 0; i < (int)drops.size(); i++)
	{
		if (drops[i].type == wtSnow)
		{
			drops[i].y = (int)(((float)t) * drops[i].speed) + drops[i].y;
			drops[i].x += rand() % 4 - 2;
			int w, h;
			engine->getWindowSize(&w, &h);
		...
		{
			drops[i].y = (int)(((float)t) * (drops[i].speed > 0.4f ? drops[i].speed : 0.4f)) + drops[i].y;
			int w, h;
			engine->getWindowSize(&w, &h);
			if (drops[i].x < - dropRange || drops[i].x > w + dropRange || drops[i].y > h + dropRange)
			{
