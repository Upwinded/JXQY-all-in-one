...
void TitleTeam::onDraw()
{
	...
	engine->drawUnicodeText(L"½£ÏÀÇéÔµ·¡", w - 370, 90, 55, color);
	engine->drawUnicodeText(L"ÒýÇæÖØÖÆ£º", w - 410, 200, 30, color);
	engine->drawUnicodeText(L"Upwinded", w - 300, 270, 26, color);
	engine->drawUnicodeText(L"ÌØ±ð¸ÐÐ»£º", w - 410, 340, 30, color);
	engine->drawUnicodeText(L"Å¼Ïñ(Weyl¡¢BT¡¢scarsty¡¢SB500)", w - 410, 425, 26, color);
	engine->drawUnicodeText(L"Ð¡ÊÔµ¶½£", w - 290, 480, 26, color);
	engine->drawUnicodeText(L"´óÎäÏÀÂÛÌ³(dawuxia.net)", w - 385, 535, 26, color);
	engine->drawUnicodeText(L"½£ÏÀÇéÔµÌù°É", w - 320, 590, 26, color);
}
